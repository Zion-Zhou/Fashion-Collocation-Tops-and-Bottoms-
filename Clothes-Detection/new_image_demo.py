...
#while(True):
for path,dir_list,file_list in imgs:  
    for file_name in file_list:  
        ...
        if len(detections) != 0 :
            ...
            for x1, y1, x2, y2, cls_conf, cls_pred in detections:
                ...
                cropObj = img[y1-10:y2+10, x1-10:x2+10]
                ...
                obj_type = classes[int(cls_pred)]
                img_id = file_path.split('/')[-1].split('.')[0]
                file_name = img_id
                if obj_type in top_cloth:
                    file_name += "_top"
                    cv2.imwrite('output/{}.jpg'.format(file_name), cropObj)
                elif obj_type in bottom_cloth:
                    file_name += "_bottom"
                    cv2.imwrite('output/{}.jpg'.format(file_name), cropObj)
                else:
                    print("Not classifed in top or bottom...")
                    print(obj_type)
